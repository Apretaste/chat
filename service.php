...
class Nota extends Service
{
    ...
    public function _main (Request $request)
    {
        ...
        if ($un === false) {
            ...
            $contacts = $db->deepQuery(
                    "SELECT (select username FROM person WHERE person.email = subq.username) as username,  
            ...
            if (is_array($contacts))
                foreach ($contacts as $k => $contact) {
                    $last_note = $this->getConversation($person->email, 
                            $contact->email, 1);
            ...
            $response->createFromTemplate("nouser.tpl", 
                    array(
                            "contacts" => $contacts
                    ));
            return $response;
        ...
        $friend = false;
        $find = $db->deepQuery(
                "SELECT email FROM person WHERE username = '$un';");
        
        // The user $un not exists
        if (! isset($find[0])) {
            $response = new Response();
            $response->setResponseSubject(
                    "No se pudo enviar la nota pues el usuario @$un no existe");
        ...
        if ($nt !== false) {
            ...
            $db->deepQuery(
                    "INSERT INTO _note (from_user, to_user, text) VALUES ('{$request->email}','{$friend->email}','$nt');");
            ...
            $response->setResponseSubject(
                    "@{$person->username} te envio una nota");
            ...
            $response2->createFromTemplate("basic.tpl", 
                    array(
                            'username' => $un,
                            'notes' => $notes
                    ));
